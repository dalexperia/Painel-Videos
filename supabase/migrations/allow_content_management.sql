/*
  # Grant Full Table Access for Content Management

  This migration configures the security policies for the `apostilas_content` table to allow the application to manage video content.
  ...
  1. Security Changes
    ...
    - **Create Access Policy**: A new policy named "Enable all actions for anonymous users" is created. This policy grants full permissions (`SELECT`, `INSERT`, `UPDATE`, `DELETE`) to the `anon` role, which is used by the application's frontend client. This is necessary for the gallery, reprove, restore, and delete functionalities to work correctly.
*/
...
ALTER TABLE public.apostilas_content ENABLE ROW LEVEL SECURITY;
...
BEGIN
  IF NOT EXISTS (
    ...
    FROM pg_policy
    WHERE
      polname = 'Enable all actions for anonymous users' AND
      polrelid = 'public.apostilas_content'::regclass
  ) THEN
    CREATE POLICY "Enable all actions for anonymous users"
    ON public.apostilas_content
